...
async def sqlacodegen_crud(tables: str, force: bool, db_url: str) -> None:
    template_dir = (
    ...
    for table_name in tables:
        # 检查是否为合法的数据库表名（字母开头，包含字母、数字、下划线）
        if table_name.replace("_", "").isalnum() and table_name[0].isalpha():
            # 分两种情况处理
            if "_" in table_name:
                # 情况1：包含下划线的表名，提取第一个下划线前的部分作为模块名
                module_name = table_name.split("_")[0]
            else:
                # 情况2：纯字母数字的表名，整个表名作为模块名
                module_name = table_name
        else:
            typer.echo(
                typer.style(
                    f"警告: 表名 {table_name} 不是合法的数据库表名，跳过此表",
        ...
        # 验证输出目录存在
        plugin_output_dir = settings.PLUGIN_PATH.joinpath(module_name)
        app_output_dir = settings.APP_PATH.joinpath(module_name)
        if plugin_output_dir.exists():
            output_dir = plugin_output_dir
        elif app_output_dir.exists():
            output_dir = app_output_dir
        else:
            typer.echo(
                typer.style(f"{output_dir} 不存在", fg=typer.colors.YELLOW, bold=True)
            )
            continue
        ...
        variables["model_name"] = pascal_table_name  # 去掉前缀
        ...
        variables["api_path"] = not_prefix_table.replace("_", "-")
        ...
        typer.echo(
            typer.style(
                f"数据表 '{table_name}' 生成代码文件{success_count}个。",
...
def crud(
    ...
    output: str = typer.Option(None, "--out", "-o", help="output path"),
):
    asyncio.run(sqlacodegen_crud(tables, force, db_url))
