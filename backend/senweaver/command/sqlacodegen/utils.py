...
def uses_default_name(constraint: Constraint | Index) -> bool:
    ...
    if isinstance(constraint, (Index, ColumnCollectionConstraint)):
        values.update({
            ...
            'column_0N_key': ''.join(col.key for col in constraint.columns if col.key),
            'column_0_N_key': '_'.join(col.key for col in constraint.columns if col.key),
        })
    ...
    if isinstance(constraint, Index):
        ...
        values.update({
            ...
            'referred_column_0_label': constraint.elements[0].column.label(constraint.elements[0].column.name).name,
            'referred_fk.column_0N_label': ''.join(fk.column.label(fk.column.name).name for fk in constraint.elements),
            ...
            'referred_fk.column_0N_key': ''.join(fk.column.key for fk in constraint.elements if fk.column.key),
            'referred_fk.column_0_N_key': '_'.join(fk.column.key for fk in constraint.elements if fk.column.key),
        })
    else:
