...
class CodeGenerator(metaclass=ABCMeta):
    ...
    def generate_one(self) -> dict[str, object]:
        """
        Generate the code for the given metadata.
        .. note:: May modify the metadata.
        """
...
class TablesGenerator(CodeGenerator):
    ...
    def add_import(self, obj: Any) -> None:
        ...
        #     pkgname = "sqlalchemy"
        if type_.__name__ in dir(sqlalchemy):
            pkgname = "sqlalchemy"
        else:
            pkgname = type_.__module__
    ...
    def render_constraint(self, constraint: Constraint | ForeignKey) -> str:
        ...
        if isinstance(constraint, ForeignKey):
            ...
            raise TypeError(
                f"Cannot render constraint of type {
    ...
    def fix_column_types(self, table: Table) -> None:
        ...
        for column in table.c:
            try:
                colname = column.name  # is_开头且是TINYINT(1)
                if colname.startswith("is_") or colname == "enabled":
                    table.c[colname].type = Boolean()
                if isinstance(table.c[colname].type, DateTime):
                    table.c[colname].type = DateTime(timezone=True)
...
class DeclarativeGenerator(TablesGenerator):
    ...
    def render_table_args(
        self, table: Table, return_as_str: bool = True
    ) -> Union[str, list[str]]:
        args: list[str] = []
        kwargs: dict[str, str] = {}
    ...
    def render_relationship(self, relationship: RelationshipAttribute) -> str:
        ...
        def render_join(terms: list[JoinType]) -> str:
            ...
            for source, source_col, target, target_col in terms:
                rendered = f"lambda: {source.name}.{
                    source_col} == {target.name}."
        ...
        if relationship.association_table:
            ...
            if relationship.association_table.schema:
                table_ref = f"{
                    relationship.association_table.schema}.{table_ref}"
...
class SQLModelGenerator(DeclarativeGenerator):
    ...
    def generate_one(self) -> dict[str, object]:
        ...
        model = models[0]
        sections["model_name"] = model.name
        sections["model_columns"] = model.columns
        sections["table_args"] = self.render_table_args(model.table, False)
        sections["table_is_tree"] = False
        rendered_column_attributes: list[str] = []
        sections["web_columns"] = []
        for column_attr in model.columns:
            ...
            if column_name in [
                ...
                continue
            if column_name not in ["is_deleted", "deleted_time"]:
                sections["web_columns"].append(column_attr)
            if column_name == "parent_id":
                sections["table_is_tree"] = True
            rendered_column_attributes.append(self.render_column_attribute(column_attr))

        sections["rendered_column_attributes"] = rendered_column_attributes
        ...
        sections["rendered_relationship_attributes"] = rendered_relationship_attributes

        sections["declaration"] = self.render_class_declaration(model)
    ...
    def collect_imports(self, models: Iterable[Model]) -> None:
        ...
        if any(isinstance(model, ModelClass) for model in models):
            self.remove_literal_import("sqlalchemy", "MetaData")
            self.add_literal_import("senweaver.db.models", "SQLModel")
            self.add_literal_import("senweaver.db.models", "Field")
    ...
    def render_column_attribute(self, column_attr: ColumnAttribute) -> str:
        ...
        rendered_column = self.render_column(column, False, True)
